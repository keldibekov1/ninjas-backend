  ...
  import { DeleteShiftsDto, FinishJobDto, ClockOutDto, ClockInDto, AddShiftsDto, UpdateShiftsDto } from './shifts.dto';
  ...
  async clockIn({ tenantId, workerId, clockin_time }: ClockInDto & { tenantId: number }) {
      ...
      if (currentShift.hasShift) {
        if (currentShift.shiftStatus === 'completed') {
          throw new BadRequestException('Worker has already completed a shift today');
        }
    ...
    async finishJob(finishJobDto: FinishJobDto, tenantId: number) {
      ...
      if (!shift) {
        throw new NotFoundException(`Shift with ID ${id} not found or unauthorized`);
      }
    ...
    async shiftsUpdate(updateShiftsDto: UpdateShiftsDto) {
      const { id, clockin_time, finishjob_time, clockout_time, date } = updateShiftsDto;
    ...
    async addShift(addShiftsDto: AddShiftsDto ) {
      const {tenantId, workerId, clockin_time, finishjob_time, clockout_time, date } = addShiftsDto;
